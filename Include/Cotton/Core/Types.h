...
// MSVC provides fixed-size integer types with the __intN type specifier.
    ...
    #elif defined(_ILP32) || defined(__ILP32__) || (defined(__fourbyteints__) && __fourbyteints__) ||
          (defined(_WIN32) && !defined(_WIN64))     // Make it clear that we're checking for 32-bits.
    ...
    // Specify that whoever owns this 'Owner' object is responsible for freeing it.
    // Purely syntactical, doesn't affect runtime.
    template<class T>
    using Owner = T;
